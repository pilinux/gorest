...
func RedisCreate(c *gin.Context) {
	...
	if result != "OK" {
		render(c, gin.H{"msg": "operation failed"}, http.StatusNotAcceptable)
		return
	}

	render(c, data, http.StatusOK)
}
...
func RedisRead(c *gin.Context) {
	...
	if result == 0 {
		render(c, gin.H{"msg": "key does not exist"}, http.StatusNotFound)
		return
	}
...
func RedisDelete(c *gin.Context) {
	...
	render(c, gin.H{"msg": "key is deleted"}, http.StatusOK)
}
